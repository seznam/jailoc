...
**🔄 Docker Compose orchestrace** — dvě služby jsou spravované přes [Compose Go SDK](https://github.com/docker/compose): služba `opencode` (kontejner s agentem) a sidecar `dind`, který poskytuje izolovaný Docker daemon — běží hluboko ve své kovárně jako Hephaestus. Agent komunikuje s DinD daemonem přes TLS pomocí sdíleného named volume pro certifikáty. Žádný hostitelský Docker socket se nepřipojuje.

**🚪 Entrypoint** — kontejner nastartuje jako root (jako Sisyphus — první na místě, všechno na bedrech), aby mohl nastavit iptables pravidla a chownout datový volume. Pak přejde na UID 1000 (`agent`) přes `setpriv --inh-caps=-all --no-new-privs` a spustí OpenCode server process.
