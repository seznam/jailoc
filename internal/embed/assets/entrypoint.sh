...
# config directory on startup. Inside containers, this directory may not exist
# or may be read-only, causing a fatal EROFS error. Pre-creating it here
# (as root, before privilege drop) prevents the crash for the default case
# (no user-mounted config directory).
# See: https://github.com/anomalyco/opencode/issues/23040
mkdir -p /home/agent/.config/opencode
if [ ! -f /home/agent/.config/opencode/.gitignore ]; then
  cat > /home/agent/.config/opencode/.gitignore 2>/dev/null <<'GITIGNORE' || true
# Auto-generated by jailoc entrypoint — do not edit.
# jailoc 1.13+ ships OpenCode v1.14.22+ which requires this file in every
# config directory. Without it, startup fails with EROFS on read-only
# filesystems. See: https://github.com/anomalyco/opencode/issues/23040
node_modules
package.json
package-lock.json
bun.lock
