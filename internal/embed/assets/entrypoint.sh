...
# OpenCode 1.13+ writes a .gitignore to every config directory on startup.
# Inside containers, this directory may not exist or may be read-only, causing
# a fatal EROFS error. Pre-creating it here (as root, before privilege drop)
# prevents the crash for the default case (no user-mounted config directory).
...
# OpenCode 1.13+ requires this file to exist in every config directory.
# Without it, startup fails with EROFS on read-only filesystems.
# See: https://github.com/anomalyco/opencode/issues/23040
node_modules
package.json
package-lock.json
