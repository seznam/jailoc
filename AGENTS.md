...
- `tui.js` — built TUI plugin JS (from `plugin/index.jsx` via esbuild)
...
```
The embedded `tui.js` must stay in sync with the plugin source — `go:embed` picks it up at compile time.
